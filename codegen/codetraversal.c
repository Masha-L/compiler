...
/*
	...
	only be VAR_DECs (if any) follow by FUNC_DECLs
...
void handle_program(ast_node *program) {
	...
	while (i < program->num_children && 
		program->childlist[i]->symbol->grammar_symbol == VAR_DEC) {
...
/*
	...
	the return type, id/name, possibly a PAR_DEC, and a BLOCK
...
void handle_function(ast_node *funcDecl) {
	...
	for (i=2; i < funcDecl->num_children && 
		funcDecl->childlist[i]->symbol->grammar_symbol == PAR_DEC; i++) {
...
/*
	...
	VAR_DEC (if any), followed by a bunch of statements
...
void handle_block(ast_node *block) {
	...
	while (i < block->num_children && 
	 	block->childlist[i]->symbol->grammar_symbol == VAR_DEC) {
